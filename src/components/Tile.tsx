...
export const Tile: React.FC<TileProps> = ({
  ...
  return (
    <div
      style={customStyle}
      className={`absolute w-[100px] -translate-x-1/2 -translate-y-1/2 rounded-xl p-1.5 text-center text-[10px] leading-tight border transition-transform duration-150 z-10 select-none shadow-md ${bgClasses} ${
        pulse
    ...
    >
      ...
      {continentDef && (
        <div className="flex items-center justify-between mb-1 px-1 py-0.5 rounded-md bg-black/30 border border-white/5">
          <div className="flex items-center gap-1">
            ...
            />
            <span className="text-[7.5px] uppercase tracking-wider text-zinc-300 font-semibold truncate max-w-[42px]">
              {continentDef.name.slice(0, 5)}
            </span>
          </div>
          <span className="text-[8px] text-[#c9a13b] font-mono font-bold">
            ${tile.price}
          </span>
        </div>
      )}
      ...
      {/* Start Banner */}
      {tile.type === 'start' && (
        <div className="text-[8px] uppercase tracking-wider mb-0.5 opacity-80">
          Kongres Dunia
        </div>
      )}

      {/* Tile Name */}
      <div className="font-semibold line-clamp-2 leading-tight tracking-tight text-[10.5px]">
        {tile.type === 'start' ? `★ ${tile.name}` : tile.name}
      </div>

      {/* Basis Tag */}
      {tile.type === 'basis' && (
        <div className="text-[7.5px] text-[#c9a13b] font-bold tracking-wider mt-0.5 uppercase bg-black/40 py-0.5 rounded">
          ⚡ Basis Kekuatan
        </div>
      )}
      ...
      {/* Owner Badge */}
      {ownerDef && (
        <div className="mt-1 px-1 py-0.5 rounded bg-black/40 text-[7.5px] font-bold uppercase tracking-wider truncate flex items-center justify-center gap-1">
          <span
